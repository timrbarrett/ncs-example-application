...
#include "app/etl_type/bleserial.h"

#define Serial bleserial

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(ulisp);
...
object *fn_oneplus(object *args, object *env)
{
	...
	object *arg = first(args);
	if (floatp(arg))
		return makefloat((arg->single_float) + 1.0f);
	else if (integerp(arg)) {
		int result = arg->integer;
		if (result == INT_MAX)
			return makefloat((arg->integer) + 1.0f);
		else
...
object *fn_plusp(object *args, object *env)
{
	...
	object *arg = first(args);
	if (floatp(arg))
		return ((arg->single_float) > 0.0f) ? tee : nil;
	else if (integerp(arg))
...
object *fn_log(object *args, object *env)
{
	(void)env;
	object *arg = first(args);
	double fresult = log((double)checkintfloat(arg));
	args = cdr(args);
	if (args == NULL)
		return makefloat(fresult);
	else
		return makefloat(fresult / log((double)checkintfloat(first(args))));
}
...
object *fn_expt(object *args, object *env)
{
	...
	float value = log(abs(float1)) * (double)checkintfloat(arg2);
...
/* 
	I want this to take a character from rx ringbuffer if it exists
*/
char Serialread(void)
{
	...
	err = bleserial_receive_char(&c);
	if (err==0) {
		//LOG_INF("Received char: %c\n", c);
	} else if (err == -ENODATA) {
		k_sleep(K_MSEC(10));
	} else {
		LOG_ERR("Error receiving character: %d\n", err);
	}
...
*/
void pfloat(double f, pfun_t pfun)
{
	if (isnan(f)) {
...
{
	//LOG_INF("%i processkey top",__LINE__);
	if (c == 27) {
		setflag(ESCAPE);
		return;
	} // Escape key
...
#endif
	//LOG_INF("%i processkey bottom",__LINE__);
	return;
}
...
int gserial(void)
{
	...
	while (!KybdAvailable) {
		...
		//LOG_INF("ln9419 preSerialAvailable");
		while (!Serialavailable()) ;
		//LOG_INF("ln9421 postSerialAvailable");
		char temp = Serialread();
		//LOG_INF("ln9423 postSerialRead");
...
object *nextitem(gfun_t gfun)
{
	...
	if (isbr(ch))
		LastChar = ch;
	if (isfloat && valid == 1)
		return makefloat((double)fresult * (double)sign * pow(10.0, (double)(exponent * esign)));
	else if (valid == 1) {
...
void repl(object *env)
{
	//for (;;) {
		...
		// randomSeed(micros());
		gc(NULL, env); //LOG_INF("%i gc",__LINE__);
#if defined(printfreespace)
